  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4 max-w-6xl mx-auto">
        ...
        <Button asChild className="py-6 px-8 text-lg font-semibold flex items-center space-x-2 group bg-crypto-primary text-black hover:bg-crypto-primary/80">
          <Link to="/login"><span>Zaloguj się</span><span className="group-hover:translate-x-1 transition-transform">→</span></Link>
        </Button>
      </div>
    );
  }
      ...
      <div className="relative z-10 card overflow-hidden bg-crypto-card/60 backdrop-blur-xl border border-gray-800/80 hover:box-glow-purple transition-shadow duration-500 rounded-2xl">
        {loading ? (
          <div className="p-12 text-center text-gray-500">
            <div className="flex justify-center mb-4"><div className="w-8 h-8 border-2 border-crypto-primary/30 border-t-crypto-primary rounded-full animate-spin"></div></div>
            ...
            <Table>
              <TableHeader className="bg-gray-900/20">
                <TableRow className="border-gray-800/50 hover:bg-transparent">
                  ...
                  <TableHead className="text-gray-400">Nazwa</TableHead>
                  <TableHead className="text-right text-gray-400">Cena</TableHead>
                  <TableHead className="text-right text-gray-400">Zmiana 24h</TableHead>
                  <TableHead className="text-center w-24 text-gray-400">Akcje</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {favoriteCryptos.map((coin) => {
                  ...
                  return (
                    <TableRow key={coin.id} onClick={() => navigate(`/coin/${coin.id}`)}
                      className="hover:bg-crypto-purple/10 transition-all cursor-pointer duration-300 group border-gray-800/30 data-[state=selected]:bg-transparent hover:shadow-[inset_4px_0_0_rgba(176,38,255,1)]">
                      <TableCell>
                        <button onClick={(e) => { e.stopPropagation(); handleToggleStar(coin.id); }}
                      ...
                      <TableCell className="flex items-center space-x-4">
                        <div className="w-10 h-10 rounded-full flex flex-shrink-0 items-center justify-center bg-gray-800 overflow-hidden group-hover:shadow-[0_0_15px_rgba(176,38,255,0.6)] transition-shadow duration-300">
                          {coin.image ? <img src={coin.image} alt={coin.name} className="w-full h-full object-cover" /> : <span className="text-sm font-bold text-gray-300">{coin.symbol[0]}</span>}
                        </div>
                        <div>
                      ...
                      <TableCell className="text-center">
                        <Button variant="ghost" onClick={(e) => { e.stopPropagation(); navigate(`/coin/${coin.id}`); }}
                          className="text-gray-400 hover:text-crypto-purple transition-colors rounded-full hover:bg-crypto-purple/20 p-2 h-auto">
                          <BarChart2 size={18} />
                        </Button>
                      </TableCell>
                    </TableRow>
          ...
          <div className="p-16 text-center text-gray-400 flex flex-col items-center">
            ...
            <Button asChild className="bg-crypto-primary hover:bg-crypto-primary/80 text-black font-bold px-8 py-6 text-lg">
              <Link to="/">Przeglądaj Rynek</Link>
            </Button>
          </div>
        )}
      </div>
